...
from manager.models import Playlist, PlaylistEntry, import_playlists, Track, Artist
...
def playing(request):
    ...
    playlist = get_object_or_404(
        Playlist, DeezerId=int(request.POST['choice']))
    ...
    current_track = get_object_or_404(
        Track, DeezerId=track_list[0].TrackId.DeezerId)
...
def track_playing(request):
    ...
    current_track = get_object_or_404(
        Track, DeezerId=track_list[request.session.get('indice') + 1].TrackId.DeezerId)
...
def test_design(request):
    return render(request, 'player/neon_player.html', {})


def current_track_list(request):
    playlist_id = request.session.get('playlist_id')
    playlist = get_object_or_404(Playlist, DeezerId=int(playlist_id))

    track_list = PlaylistEntry.objects.all().filter(PlaylistId=playlist.id)
    dplaylist = playlist.to_dict()
    # current_track = Track.objects.get(DeezerId=track_list[request.session.get('indice') + 1].TrackId.DeezerId)
    ...
    jsData = json.dumps(data, sort_keys=True, indent=2)
